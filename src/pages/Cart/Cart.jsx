...
const Cart = () => {
  ...
  const handleViewMenuClick = () => {
    navigate("/#explore-menu"); // Add the hash to the URL
  };
  if (getTotalCartAmount() === 0) {
    return (
      <div className="no-cart">
        <img src={assets.no_food} alt="" />

        <div className="no-cart-items">Your cart is empty</div>
        <div className="see-rest">
          You can go to home page to view more dishes
        </div>
        <button className="go-to-menu" onClick={handleViewMenuClick}>
          View Menu
        </button>
      </div>
    );
